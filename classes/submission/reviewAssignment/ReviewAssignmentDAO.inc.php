...
class ReviewAssignmentDAO extends DAO {
	...
	function &getPresenterViewableFilesByStage($paperId) {
		...
		while (!$result->EOF) {
			...
			if (!isset($files[$row['stage']][$reviewer_index]) || !is_array($files[$row['stage']][$reviewer_index])) {
				$files[$row['stage']][$reviewer_index] = array();
			}
			$thisPaperFile = &$this->paperFileDao->_returnPaperFileFromRow($row);
			$files[$row['stage']][$reviewer_index][] = $thisPaperFile;
			$result->MoveNext();
		}
	...
	function insertReviewAssignment(&$reviewAssignment) {
		$this->update(
			sprintf('INSERT INTO review_assignments
				(paper_id, reviewer_id, stage, recommendation, declined, replaced, cancelled, date_assigned, date_notified, date_confirmed, date_completed, date_acknowledged, date_due, reviewer_file_id, quality, date_rated, last_modified, date_reminded, reminder_was_automatic)
				VALUES
				(?, ?, ?, ?, ?, ?, ?, %s, %s, %s, %s, %s, %s, ?, ?, %s, %s, %s, ?)',
			...
			array(
				...
				$reviewAssignment->getStage() === null ? 1 : $reviewAssignment->getStage(),
