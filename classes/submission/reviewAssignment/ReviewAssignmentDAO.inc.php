...
class ReviewAssignmentDAO extends DAO {
	 ...
	 */
	function &getReviewAssignment($paperId, $reviewerId, $type, $round) {
		$result = &$this->retrieve(
			'SELECT r.*, r2.review_revision, a.review_file_id, u.first_name, u.last_name FROM review_assignments r LEFT JOIN users u ON (r.reviewer_id = u.user_id) LEFT JOIN review_rounds r2 ON (r.paper_id = r2.paper_id AND r.round = r2.round AND r.type = r2.type) LEFT JOIN papers a ON (r.paper_id = a.paper_id) WHERE r.paper_id = ? AND r.reviewer_id = ? AND r.cancelled <> 1 AND r.type = ? AND r.round = ?',
	...
	function &getReviewAssignmentById($reviewId) {
		$result = &$this->retrieve(
			'SELECT r.*, r2.review_revision, a.review_file_id, u.first_name, u.last_name FROM review_assignments r LEFT JOIN users u ON (r.reviewer_id = u.user_id) LEFT JOIN review_rounds r2 ON (r.paper_id = r2.paper_id AND r.round = r2.round AND r.type = r2.type) LEFT JOIN papers a ON (r.paper_id = a.paper_id) WHERE r.review_id = ?',
			$reviewId
			);
	...
	/**
	 * Determine the order of active reviews for the given round of the given paper
	 * @param $paperId int
	 * @param $round int
	 * @return array associating review ID with number; ie if review ID 26 is first, returned['26']=0
	 */
	function &getReviewIndexesForRound($paperId, $type, $round) {
		$returner = array();
		$index = 0;
		$result = &$this->retrieve(
			'SELECT review_id FROM review_assignments WHERE paper_id = ? and type = ? AND round = ? AND (cancelled = 0 OR cancelled IS NULL) ORDER BY review_id',
			array($paperId, $type, $round)
			);
	...
	function &getIncompleteReviewAssignments() {
		...
		$result = &$this->retrieve(
			'SELECT r.*, r2.review_revision, a.review_file_id, u.first_name, u.last_name FROM review_assignments r LEFT JOIN users u ON (r.reviewer_id = u.user_id) LEFT JOIN review_rounds r2 ON (r.paper_id = r2.paper_id AND r.round = r2.round AND r.type = r2.type) LEFT JOIN papers a ON (r.paper_id = a.paper_id) WHERE (r.cancelled IS NULL OR r.cancelled = 0) AND r.date_notified IS NOT NULL AND r.date_completed IS NULL ORDER BY r.paper_id'
		);
	 ...
	 * @return array ReviewAssignments
	 */
	function &getReviewAssignmentsByPaperId($paperId, $type = null, $round = null) {
		$reviewAssignments = array();

		$args = array_diff(array($paperId, $type, $round), array(null));
		...
		$result = &$this->retrieve('
			...
			WHERE r.paper_id = ? '
				. ($type ? ' AND r.type = ?':'')
				. ($round ? ' AND r.round = ?':'')
			. ' ORDER BY ' . ($type?'':' type,') . ($round?'':' round,') . 'review_id',
	...
	function &getPresenterViewableFilesByRound($paperId,$type) {
		...
		while (!$result->EOF) {
			...
			$files[$row['round']][$reviewer_index][] = $thisPaperFile;
			$result->MoveNext();
		}
	 ...
	 */
	function &getLastModifiedByRound($paperId, $type) {
		$returner = array();

		$result = &$this->retrieve(
			'SELECT round, MAX(last_modified) as last_modified FROM review_assignments WHERE paper_id=? AND type=? GROUP BY round', 
			array($paperId,$type)
		);
	 ...
	 */
	function &getEarliestNotificationByRound($paperId, $type) {
		$returner = array();

		$result = &$this->retrieve(
			'SELECT round, MIN(date_notified) as earliest_date FROM review_assignments WHERE paper_id=? AND type=? GROUP BY round', 
			array($paperId,$type)
		);
	...
	function &getCancelsAndRegrets($paperId) {
		...
		$result = &$this->retrieve(
			'SELECT r.*, r2.review_revision, a.review_file_id, u.first_name, u.last_name FROM review_assignments r LEFT JOIN users u ON (r.reviewer_id = u.user_id) LEFT JOIN review_rounds r2 ON (r.paper_id = r2.paper_id AND r.round = r2.round AND r.type = r2.type) LEFT JOIN papers a ON (r.paper_id = a.paper_id) WHERE r.paper_id = ? AND (r.cancelled = 1 OR r.declined = 1) ORDER BY round, review_id',
			$paperId
		);
	...
	function &_returnReviewAssignmentFromRow(&$row) {
		...
		$reviewAssignment->setType($row['type']);
	...
	function insertReviewAssignment(&$reviewAssignment) {
		$this->update(
			sprintf('INSERT INTO review_assignments
				(paper_id, reviewer_id, round, type, recommendation, declined, replaced, cancelled, date_assigned, date_notified, date_confirmed, date_completed, date_acknowledged, date_due, reviewer_file_id, quality, date_rated, last_modified, date_reminded, reminder_was_automatic)
				VALUES
				(?, ?, ?, ?, ?, ?, ?, ?, %s, %s, %s, %s, %s, %s, ?, ?, %s, %s, %s, ?)',
			...
			array(
				$reviewAssignment->getPaperId(),
				$reviewAssignment->getReviewerId(),
				$reviewAssignment->getRound() === null ? 1 : $reviewAssignment->getRound(),
				$reviewAssignment->getType() === null ? 1 : $reviewAssignment->getType(),
	...
	function deleteReviewAssignmentsByPaper($paperId) {
		return $this->update(
			'DELETE FROM review_assignments WHERE paper_id = ?',
			$paperId
		);
	}
