...
class UserSettingsDAO extends DAO {
	...
	function &getSetting($userId, $name, $conferenceId = null) {
		...
		if ($conferenceId == null) {
			$result = &$this->retrieve(
				'SELECT setting_value, setting_type FROM user_settings WHERE user_id = ? AND setting_name = ? AND conference_id IS NULL', array($userId, $name)
			);
		} else {
			$result = &$this->retrieve(
	...
	function &getUsersBySetting($name, $value, $type = null, $conferenceId = null) {
		...
		if ($conferenceId == null) {
			$result = &$this->retrieve(
				'SELECT u.* FROM users u, user_settings s WHERE u.user_id = s.user_id AND s.setting_name = ? AND s.setting_value = ? AND s.conference_id IS NULL',
				array($name, $value)
			);
		} else {				
			$result = &$this->retrieve(
	...
	function &getSettingsByConference($userId, $conferenceId = null) {
		...
		if ($conferenceId == null) {
			$result = &$this->retrieve(
				'SELECT setting_name, setting_value, setting_type FROM user_settings WHERE user_id = ? AND conference_id IS NULL', $userId
			);
		} else {
			$result = &$this->retrieve(
	...
	function updateSetting($userId, $name, $value, $type = null, $conferenceId = null) {
		...
		if ($conferenceId == null) {		
			$result = $this->retrieve(
				'SELECT COUNT(*) FROM user_settings WHERE user_id = ? AND setting_name = ? AND conference_id IS NULL', array($userId, $name)
			);
		} else {
			$result = $this->retrieve(
			...
			if ($conferenceId == null) {
				$returner = $this->update(
					'UPDATE user_settings SET
						setting_value = ?,
						setting_type = ?
						WHERE user_id = ? AND setting_name = ? AND conference_id IS NULL',
	...
	function deleteSetting($userId, $name, $conferenceId = null) {
		if ($conferenceId == null) {
			return $this->update(
				'DELETE FROM user_settings WHERE user_id = ? AND setting_name = ? AND conference_id IS NULL',
				array($userId, $name)
			);
		} else {		
			return $this->update(
