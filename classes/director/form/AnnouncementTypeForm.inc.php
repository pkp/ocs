 ...
 *
 * Form for conference directors to create/edit announcement types.
 *
 * $Id$
 */
...
class AnnouncementTypeForm extends Form {
	...
	function AnnouncementTypeForm($typeId = null) {
		...
		parent::Form('director/announcement/announcementTypeForm.tpl');

		// Type name is provided
		$this->addCheck(new FormValidator($this, 'typeName', 'required', 'director.announcementTypes.form.typeNameRequired'));

		// Type name does not already exist for this conference
		if ($this->typeId == null) {
			$this->addCheck(new FormValidatorCustom($this, 'typeName', 'required', 'director.announcementTypes.form.typeNameExists', array(DAORegistry::getDAO('AnnouncementTypeDAO'), 'announcementTypeExistsByTypeName'), array($conference->getConferenceId()), true));
		} else {
			$this->addCheck(new FormValidatorCustom($this, 'typeName', 'required', 'director.announcementTypes.form.typeNameExists', create_function('$typeName, $conferenceId, $typeId', '$announcementTypeDao = &DAORegistry::getDAO(\'AnnouncementTypeDAO\'); $checkId = $announcementTypeDao->getAnnouncementTypeByTypeName($typeName, $conferenceId); return ($checkId == 0 || $checkId == $typeId) ? true : false;'), array($conference->getConferenceId(), $this->typeId)));
		}
	}
