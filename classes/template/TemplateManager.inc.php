...
class TemplateManager extends Smarty {
		...
		if (!defined('SESSION_DISABLE_INIT')) {
			...
			if (isset($conference)) {
				...
				if (isset($schedConf)) {
					...
					if ($schedConf->getSetting('postSchedule') && $currentTime > $postScheduleDate) {
						$this->assign('schedConfShowSchedule', true);
					}
	...
	function smartyPageLinks($params, &$smarty) {
		...
		if ($page>1) {
			$params[$paramName] = 1;
			$value .= '<a href="' . Request::url(null, null, null, null, Request::getRequestedArgs(), $params, $anchor) . '"' . $allExtra . '>&lt;&lt;</a>&nbsp;';
		...
		for ($i=$pageBase; $i<min($pageBase+$numPageLinks, $pageCount+1); $i++) {
			if ($i == $page) {
				...
				$params[$paramName] = $i;
				$value .= '<a href="' . Request::url(null, null, null, null, Request::getRequestedArgs(), $params, $anchor) . '"' . $allExtra . '>' . $i . '</a>&nbsp;';
			}
		}
		if ($page < $pageCount) {
			$params[$paramName] = $page + 1;
