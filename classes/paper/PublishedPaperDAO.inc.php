...
class PublishedPaperDAO extends DAO {
	...
	function &getPublishedPaperByPaperId($paperId, $schedConfId = null, $previewAbstracts = null) {
		...
		$result =& $this->retrieve(
			...
			WHERE	a.paper_id = ?' .
				...
				($previewAbstracts===true?' AND (a.status = ' . STATUS_PUBLISHED . ' OR (a.review_mode <> ' . REVIEW_MODE_BOTH_SIMULTANEOUS . ' AND a.status = ' . STATUS_QUEUED . ' AND a.current_round = ' . REVIEW_ROUND_PRESENTATION . '))':'') .
	...
	function insertPublishedPaper(&$publishedPaper) {
		...
		$publishedPaper->setPubId($this->getInsertId());
