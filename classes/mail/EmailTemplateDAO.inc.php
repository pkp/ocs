	...
	function &getLocaleEmailTemplate($emailKey, $conferenceId, $schedConfId = 0, $allowConferenceTemplates = true) {
		if($allowConferenceTemplates && $schedConfId != 0) {
			$result = &$this->retrieve(
				...
				FROM	email_templates_default d
					LEFT JOIN email_templates e ON (d.email_key = e.email_key AND e.conference_id = ? AND e.sched_conf_id = ?)
					LEFT JOIN email_templates e2 ON (d.email_key = e2.email_key AND e2.conference_id = ? AND e2.sched_conf_id = 0)
				WHERE	d.email_key = ?',
		...
		if (!HookRegistry::call('EmailTemplateDAO::_returnLocaleEmailTemplateFromRow', array(&$emailTemplate, &$row))) {
			$result = &$this->retrieve(
				...
				FROM	email_templates_default_data dd
					LEFT JOIN email_templates_data ed ON (dd.email_key = ed.email_key AND dd.locale = ed.locale AND ed.conference_id = ? AND ed.sched_conf_id = ?)
				WHERE	dd.email_key = ?',
			...
			$result = &$this->retrieve(
				...
				WHERE	ed.conference_id = ? AND
					ed.sched_conf_id = ? AND
					ed.email_key = ? AND
	...
	function &getEmailTemplates($locale, $conferenceId, $schedConfId = 0) {
		...
		$result = &$this->retrieve(
			'SELECT	ed.subject,
				...
				NULL AS to_role_id
			FROM	email_templates e
				LEFT JOIN email_templates_data ed ON (e.email_key = ed.email_key AND ed.conference_id = e.conference_id AND ed.sched_conf_id = e.sched_conf_id AND ed.locale = ?)
				LEFT JOIN email_templates_default d ON (e.email_key = d.email_key)
			WHERE	e.conference_id = ? AND
				e.sched_conf_id = ? AND
				d.email_key IS NULL',
