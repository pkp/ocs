...
{if $registrationId}
	{assign var="pageTitle" value="manager.registrations.edit"}
{else}
	{assign var="pageTitle" value="manager.registrations.create"}
{/if}
...
<tr valign="top">
	<td width="20%" class="label">{fieldLabel name="userId" required="true" key="manager.registrations.form.userId"}</td>
...
<tr valign="top">
	<td class="label">{fieldLabel name="typeId" required="true" key="manager.registrations.form.typeId"}</td>
	...
	<td class="value">
		<table width="100%">
			<tr valign="top">
				...
				<td width="95%"><label for="">{translate key="manager.registrations.form.notifyEmail"}</label></td>
			</tr>
		</table>
	</td>
</tr>
<tr valign="top">
	<td class="label">{fieldLabel name="membership" key="manager.registrations.form.membership"}</td>
	<td class="value">
		<input type="text" name="membership" value="{$membership|escape}" id="membership" size="40" maxlength="40" class="textField" />
		<br />
		<span class="instruct">{translate key="manager.registrations.form.membershipInstructions"}</span>
	</td>
</tr>
<tr valign="top">
	<td class="label">{fieldLabel name="domain" key="manager.registrations.form.domain"}</td>
	<td class="value">
		<input type="text" name="domain" value="{$domain|escape}" size="40" id="domain" maxlength="255" class="textField" />
		<br />
		<span class="instruct">{translate key="manager.registrations.form.domainInstructions"}</span>
	</td>
</tr>
<tr valign="top">
	<td class="label">{fieldLabel name="ipRange" key="manager.registrations.form.ipRange"}</td>
	<td class="value">
		<input type="text" id="ipRange" name="ipRange" value="{$ipRange|escape}" size="40" maxlength="255" class="textField" />
		<br />
		<span class="instruct">{translate key="manager.registrations.form.ipRangeInstructions"}</span>
	</td>
</tr>
<tr valign="top">
	<td class="label">{fieldLabel name="specialRequests" key="manager.registrations.form.specialRequests"}</td>
	<td class="value">
		...
		<br />
		<span class="instruct">{translate key="manager.registrations.form.specialRequestsInstructions"}</span>
	</td>
</tr>
</table>

<p><input type="submit" value="{translate key="common.save"}" class="button defaultButton" /> {if not $registrationId}<input type="submit" name="createAnother" value="{translate key="manager.registrations.form.saveAndCreateAnother"}" class="button" /> {/if}<input type="button" value="{translate key="common.cancel"}" class="button" onclick="document.location.href='{url op="registrations" escape=false}'" /></p>
