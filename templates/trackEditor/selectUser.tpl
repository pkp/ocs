...
<p>{foreach from=$alphaList item=letter}<a href="{url op=$actionHandler path=$paperId searchInitial=$letter}">{if $letter == $searchInitial}<strong>{$letter}</strong>{else}{$letter}{/if}</a> {/foreach}<a href="{url op=$actionHandler path=$paperId}">{if $searchInitial==''}<strong>{translate key="common.all"}</strong>{else}{translate key="common.all"}{/if}</a></p>
...
<tr valign="top">
	...
	<td>
		{if $currentUser != $userid}
			<a href="{url op=$actionHandler path=$paperId|to_array:$userid}" class="action">{translate key="common.assign"}</a>
		{else}
...
{else}
	<tr>
		...
		<td colspan="3" align="right">{page_links name="users" iterator=$users searchField=$searchField searchMatch=$searchMatch search=$search dateFromDay=$dateFromDay dateFromYear=$dateFromYear dateFromMonth=$dateFromMonth dateToDay=$dateToDay dateToYear=$dateToYear dateToMonth=$dateToMonth}</td>
	</tr>
{/if}
</table>
{if $backLink}
