...
<form class="pkp_form" method="post" id="searchForm" action="{url op="advancedResults"}">
...
{if $siteSearch}
<tr valign="top">
	<td class="label"><label for="searchConference">{translate key="search.withinConference"}</label></td>
	<td class="value"><select name="searchConference" id="searchConference" class="selectMenu">{html_options options=$conferenceOptions selected=$searchConference}</select></td>
</tr>
{/if}
<tr valign="top">
...
<tr valign="top">
	<td class="label"><label for="supplementaryFiles">{translate key="paper.suppFiles"}</label></td>
...
<tr valign="top">
	...
	<td class="value">{html_select_date prefix="dateTo" time=$dateTo all_extra="class=\"selectMenu\"" year_empty="" month_empty="" day_empty="" start_year="$startYear" end_year="$endYear"}</td>
</tr>
<tr valign="top">
