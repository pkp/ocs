...
{if $basicQuery}
	<form class="pkp_form" method="post" id="search" action="{url op="results"}">
	...
	<br />
{else}
	<form class="pkp_form" id="revise" action="{url op="advanced"}" method="post">
...
<tr valign="top">
	{if !$currentConference}<td><a href="{url conference=$conference->getPath()}">{$conference->getLocalizedName()|escape}</a></td>{/if}
	<td><a href="{url conference=$conference->getPath() schedConf=$schedConf->getPath()}">{$schedConf->getLocalizedName()|escape}</a></td>
