...
<table width="100%" class="listing">
	...
	{/if}
		<tr valign="top">
			<td>{if $editAssignment->getIsEditor()}{translate key="user.role.editor"}{else}{translate key="user.role.trackDirector"}{/if}</td>
			<td>
...
{if $isEditor}
	...
	<a href="{url op="assignEditor" path="trackDirector" paperId=$submission->getPaperId()}" class="action">{translate key="editor.paper.assignTrackDirector"}</a>
	| <a href="{url op="assignEditor" path="editor" paperId=$submission->getPaperId()}" class="action">{translate key="editor.paper.assignEditor"}</a>
	{if !$selfAssigned}| <a href="{url op="assignEditor" path="editor" editorId=$userId paperId=$submission->getPaperId()}" class="action">{translate key="common.addSelf"}</a>{/if}
{/if}
</form>
