...
<p>{foreach from=$alphaList item=letter}<a href="{url op="selectReviewer" path=$paperId searchInitial=$letter}">{if $letter == $searchInitial}<strong>{$letter}</strong>{else}{$letter}{/if}</a> {/foreach}<a href="{url op="selectReviewer" path=$paperId}">{if $searchInitial==''}<strong>{translate key="common.all"}</strong>{else}{translate key="common.all"}{/if}</a></p>
...
<tr valign="top">
	...
	<td>
		...
		{else}
		<a class="action" href="{url op="selectReviewer" path=$paperId|to_array:$reviewer->getUserId()}">{translate key="common.assign"}</a>
		{/if}
	</td>
</tr>
