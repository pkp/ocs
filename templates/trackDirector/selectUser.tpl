...
<form class="pkp_form" id="submit" method="post" action="{url op=$actionHandler path=$paperId}">
...
<p>{foreach from=$alphaList item=letter}<a href="{url op=$actionHandler path=$paperId searchInitial=$letter}">{if $letter == $searchInitial}<strong>{$letter|escape}</strong>{else}{$letter|escape}{/if}</a> {/foreach}<a href="{url op=$actionHandler path=$paperId}">{if $searchInitial==''}<strong>{translate key="common.all"}</strong>{else}{translate key="common.all"}{/if}</a></p>
