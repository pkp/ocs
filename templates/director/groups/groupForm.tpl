...
{assign var="pageId" value="director.groups.groupForm"}
{assign var="pageCrumbTitle" value=$pageTitle}
{include file="common/header.tpl"}

{if $group}
	<ul class="menu">
		<li class="current"><a href="{url op="editGroup" path=$group->getGroupId()}">{translate key="director.groups.editTitle"}</a></li>
		<li><a href="{url op="groupMembership" path=$group->getGroupId()}">{translate key="director.groups.membership}</a></li>
	</ul>
{/if}
...
<tr valign="top">
	<td width="20%" class="label">{fieldLabel name="title" required="true" key="director.groups.title"}</td>
	<td width="80%" class="value"><input type="text" name="title" value="{$title|escape}" size="35" maxlength="80" id="title" class="textField" /></td>
</tr>

{if $alternateLocale1}
	<tr valign="top">
		<td width="20%" class="label">{fieldLabel name="titleAlt1" required="true" key="director.groups.title"} ({$languageToggleLocales.$alternateLocale1|escape})</td>
...
{if $alternateLocale2}
	<tr valign="top">
		<td width="20%" class="label">{fieldLabel name="titleAlt2" required="true" key="director.groups.title"} ({$languageToggleLocales.$alternateLocale2|escape})</td>
