...
{if !$isNewTemplate}
	{assign var="pageTitle" value="director.emails.editEmail"}
{else}
	{assign var="pageTitle" value="director.emails.createEmail"}
{/if}
...
{if $isNewTemplate}
	<tr valign="top">
		<td width="20%" class="label">{fieldLabel name="emailKey" key="director.emails.emailKey"}</td>
...
{foreach from=$supportedLocales item=localeName key=localeKey}
	<tr valign="top"><td colspan="2">
		<h3>{translate key="director.emails.emailTemplate"} ({$localeName|escape})</h3>
	</td></tr>
...
{foreachelse}
<tr valign="top"><td colspan="2">
	<h3>{translate key="director.emails.emailTemplate"}</h3>
</td></tr>
...
{if $canDisable}
<p><input type="checkbox" name="enabled" id="emailEnabled" value="1"{if $enabled} checked="checked"{/if} /> <label for="emailEnabled">{translate key="director.emails.enabled"}</label></p>
{/if}
