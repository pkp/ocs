...
<p><a href="{url op="editUser" path=$user->getUserId()}" class="action">{translate key="director.people.editProfile"}</a></p>
...
<h4>{translate key="director.people.enrollment"}</h4>
<ul>
	...
	<li>
		...
		{if ($isConferenceManagement) ||
			...
			<a href="{url 
					...
					onclick="return confirm('{translate|escape:"javascript" key="director.people.confirmUnenroll"}')"
					class="action">
				{translate key="director.people.unenroll"}
			</a>
		{/if}
	</li>
{/section}
