 ...
 *
 * User profile form under conference management.
 *
 * $Id$
 *}

{url|assign:"currentUrl" op="people" path="all"}
{assign var="pageTitle" value="director.people"}
...
{if $userCreated}
<p>{translate key="director.people.userCreatedSuccessfully"}</p>
{/if}

<h3>{if $userId}{translate key="director.people.editProfile"}{else}{translate key="director.people.createUser"}{/if}</h3>
...
<table width="100%" class="data">
	{if not $userId}
	<tr valign="top">	
		<td class="label">{fieldLabel name="enrollAs" key="director.people.enrollUserAs"}</td>
		<td class="value">
			...
			<br />
			<span class="instruct">{translate key="director.people.enrollUserAsDescription"}</span>
		</td>
	</tr>
	<tr valign="top">
	...
	<tr valign="top">	
		<td class="label">{fieldLabel name="authId" key="director.people.authSource"}</td>
	...
	<tr valign="top">
		<td class="label">&nbsp;</td>
		<td class="value"><input type="checkbox" onclick="setGenerateRandom(this.checked)" name="generatePassword" id="generatePassword" value="1"{if $generatePassword} checked="checked"{/if} /> <label for="generatePassword">{translate key="director.people.createUserGeneratePassword"}</label></td>
	</tr>
	<tr valign="top">
		<td class="label">&nbsp;</td>
	...
	<tr valign="top">
		<td class="label">&nbsp;</td>
		<td class="value"><input type="checkbox" name="mustChangePassword" id="mustChangePassword" value="1"{if $mustChangePassword} checked="checked"{/if} /> <label for="mustChangePassword">{translate key="director.people.userMustChangePassword"}</label></td>
	</tr>
	<tr valign="top">
...
<p><input type="submit" value="{translate key="common.save"}" class="button defaultButton" /> {if not $userId}<input type="submit" name="createAnother" value="{translate key="director.people.saveAndCreateAnotherUser"}" class="button" /> {/if}<input type="button" value="{translate key="common.cancel"}" class="button" onclick="history.go(-1);" /></p>
