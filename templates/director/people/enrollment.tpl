...
function confirmAndPrompt(userId) {
	var reason = prompt('{/literal}{translate|escape:"javascript" key="director.people.confirmDisable"}{literal}');
...
<br />
{else}
<p><a href="{url path="all"}" class="action">{translate key="director.people.allUsers"}</a></p>
{/if}
...
<table width="100%" class="listing">
	...
	<tr valign="top">
		...
		<td align="right">
			{if $roleId}
			<a href="{url op="unEnroll" path=$roleId userId=$user->getUserId()}" onclick="return confirm('{translate|escape:"javascript" key="director.people.confirmUnenroll"}')" class="action">{translate key="director.people.unenroll"}</a>&nbsp;|
			{/if}
			<a href="{url op="editUser" path=$user->getUserId()}" class="action">{translate key="common.edit"}</a>
			{if $thisUser->getUserId() != $user->getUserId()}
				|&nbsp;<a href="{url op="signInAsUser" path=$user->getUserId()}" class="action">{translate key="director.people.signInAs"}</a>
				{if !$roleId}|&nbsp;<a href="{url op="removeUser" path=$user->getUserId()}" onclick="return confirm('{translate|escape:"javascript" key="director.people.confirmRemove"}')" class="action">{translate key="director.people.remove"}</a>{/if}
				{if $user->getDisabled()}
					|&nbsp;<a href="{url op="enableUser" path=$user->getUserId()}" class="action">{translate key="director.people.enable"}</a>
				{else}
					|&nbsp;<a href="javascript:confirmAndPrompt({$user->getUserId()})" class="action">{translate key="director.people.disable"}</a>
				{/if}
			{/if}
		</td>
	</tr>
...
<a href="{url op="enrollSearch" path=$roleId}" class="action">{translate key="director.people.enrollExistingUser"}</a> | <a href="{if $roleId}{url op="createUser" roleId=$roleId}{else}{url op="createUser"}{/if}" class="action">{translate key="director.people.createUser"}</a> | <a href="{url op="enrollSyncSelect" path=$rolePath}" class="action">{translate key="director.people.enrollSync"}</a>
