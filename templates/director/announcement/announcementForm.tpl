...
{if $announcementId}
	{assign var="pageTitle" value="director.announcements.edit"}
{else}
	{assign var="pageTitle" value="director.announcements.create"}
{/if}

{assign var="pageId" value="director.announcement.announcementForm"}
...
<tr valign="top">
	<td width="20%" class="label">{fieldLabel name="typeId" key="director.announcements.form.typeId"}</td>
...
<tr valign="top">
	<td class="label">{fieldLabel name="descriptionShort" required="true" key="director.announcements.form.descriptionShort"}</td>
	<td class="value"><textarea name="descriptionShort" id="descriptionShort" cols="40" rows="6" class="textArea" />{$descriptionShort|escape}</textarea>
		<br />
		<span class="instruct">{translate key="director.announcements.form.descriptionShortInstructions"}</span>
	</td>
</tr>
<tr valign="top">
	<td class="label">{fieldLabel name="description" required="true" key="director.announcements.form.description"}</td>
	<td class="value"><textarea name="description" id="description" cols="40" rows="6" class="textArea" />{$description|escape}</textarea>
		<br />
		<span class="instruct">{translate key="director.announcements.form.descriptionInstructions"}</span>
	</td>
</tr>
<tr valign="top">
	<td class="label">{fieldLabel name="dateExpire" key="director.announcements.form.dateExpire"}</td>
	<td class="value">
		{if $dateExpire != null}
		...
		{/if}
		<br />
		<span class="instruct">{translate key="director.announcements.form.dateExpireInstructions"}</span>
	</td>
</tr>
</table>

<p><input type="submit" value="{translate key="common.save"}" class="button defaultButton" /> {if not $announcementId}<input type="submit" name="createAnother" value="{translate key="director.announcements.form.saveAndCreateAnother"}" class="button" /> {/if}<input type="button" value="{translate key="common.cancel"}" class="button" onclick="document.location.href='{url op="announcements" escape=false}'" /></p>
