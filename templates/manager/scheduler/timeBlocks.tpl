...
<table width="100%" class="listing">
	...
	<tr class="heading" valign="bottom">
		...
		<td width="25%">{translate key="common.time"}</td>
...
{iterate from=timeBlocks item=timeBlock}
	...
	{if $lastDate}{* If this isn't the first row *}
		<tr>
			<td colspan="4" class="{if $lastDate != $thisDate}end{/if}separator">&nbsp;</td>
		</tr>
	{/if}
	<tr valign="top">
		<td>{$timeBlock->getTimeBlockName()|escape}</td>
...
{else}
	<tr>
		...
		<td colspan="3" align="right">{page_links anchor="timeBlocks" name="timeBlocks" iterator=$timeBlocks}</td>
	</tr>
{/if}
</table>
