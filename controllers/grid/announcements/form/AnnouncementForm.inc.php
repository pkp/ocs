...
class AnnouncementForm extends PKPAnnouncementForm {
	 ...
	 */
	function AnnouncementForm($conferenceId, $announcementId = null, $readOnly = false) {
		...
		$this->addCheck(new FormValidatorCustom($this, 'typeId', 'optional', 'manager.announcements.form.typeIdValid', create_function('$typeId, $conferenceId', '$announcementTypeDao = DAORegistry::getDAO(\'AnnouncementTypeDAO\'); if((int)$typeId === 0) { return true; } else { return $announcementTypeDao->announcementTypeExistsByTypeId($typeId, ASSOC_TYPE_CONFERENCE, $conferenceId);}'), array($conferenceId)));
	}
