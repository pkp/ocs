...
class BackupPlugin extends GenericPlugin {
	...
	function addLink($hookName, $args) {
		...
		$output .= '<li>&#187; <a href="' . $request->url(null, null, 'backup') . '">' . __('plugins.generic.backup.link') . '</a></li>';
		return false;
	}
	...
	function handleRequest($hookName, $args) {
		...
		switch ($op) {
			...
			case 'failure':
				...
				$templateMgr->assign('backLink', $request->url(null, null, null, 'backup'));
	 ...
	 */
	function getEnabled() {
		return $this->getSetting(0, 0, 'enabled');
	}
