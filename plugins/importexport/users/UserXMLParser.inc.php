...
class UserXMLParser {
	...
	/** @var int the ID of the sched conf to import users into */
	var $schedConfId;

	/** @var int the ID of the scheduled conference to import users into */
	var $conferenceId;
	...
	function importUsers($sendNotify = false, $continueOnError = false) {
		...
		for ($i=0, $count=count($this->usersToImport); $i < $count; $i++) {
			...
			foreach ($user->getRoles() as $role) {
				...
				if (!$roleDao->userHasRole($role->getConferenceId(), $role->getSchedConfId(), $role->getUserId(), $role->getRoleId())) {
