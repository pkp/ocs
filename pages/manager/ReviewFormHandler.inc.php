...
class ReviewFormHandler extends ManagerHandler {
	...
	function deleteReviewForm($args) {
		...
		if (isset($reviewForm) && $reviewForm->getCompleteCount() == 0 && $reviewForm->getIncompleteCount() == 0) {
			...
			$reviewFormDao->deleteReviewFormById($reviewFormId, $conference->getConferenceId());
		}
	...
	function copyReviewForm($args) {
		...
		if (isset($reviewForm)) {
			...
			$reviewFormDao->resequenceReviewForms($conference->getConferenceId());
	...
	function moveReviewForm() {
		...
		$conference =& Request::getConference();
		$reviewFormDao =& DAORegistry::getDAO('ReviewFormDAO');
		$reviewForm =& $reviewFormDao->getReviewForm(Request::getUserVar('reviewFormId'), $conference->getConferenceId());
	...
	function updateReviewFormElement() {
		...
		if (!$reviewFormDao->unusedReviewFormExists($reviewFormId, $conference->getConferenceId()) || ($reviewFormElementId != null && !$reviewFormElementDao->reviewFormElementExists($reviewFormElementId, $reviewFormId))) {
			Request::redirect(null, null, null, 'reviewFormElements', array($reviewFormId));
		}
	...
	function deleteReviewFormElement($args) {
		...
		if ($reviewFormDao->unusedReviewFormExists($reviewFormId, $conference->getConferenceId())) {
			$reviewFormElementDao =& DAORegistry::getDAO('ReviewFormElementDAO');
	...
	function moveReviewFormElement() {
		...
		if (isset($reviewFormElement) && $reviewFormDao->unusedReviewFormExists($reviewFormElement->getReviewFormId(), $conference->getConferenceId())) {
