...
class EventDirectorSetupHandler extends DirectorHandler {
	...
	function setup($args) {
		...
		if ($step >= 1 && $step <= 6) {
			...
			import("director.form.eventSetup.$formClass");
			...
			$templateMgr->display('director/eventSetup/index.tpl');
		}
	}
	...
	function saveSetup($args) {
		...
		if ($step >= 1 && $step <= 6) {
			...
			if (!isset($editData) && $setupForm->validate()) {
				...
				$templateMgr->assign('helpTopicId', 'conference.managementPages.setup');
				$templateMgr->display('director/eventSetup/settingsSaved.tpl');
